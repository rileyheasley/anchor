...
import { ChevronRight, ChevronDown, Folder, Workflow, Trash2 } from 'lucide-react'
import type { Canvas, CanvasFolder } from '../types'
import { sortByMode, type CanvasSortMode } from '../utils/canvasTree'

export interface CanvasesTreeHandlers {
  activeCanvasId: string | null
  selectedCanvasIds: Set<string>
  expandedIds: Set<string>
  onToggleExpand: (id: string) => void
  onCanvasClick: (canvas: Canvas, e: React.MouseEvent) => void
  onCanvasContextMenu: (e: React.MouseEvent, canvas: Canvas) => void
  onCanvasDelete: (id: string) => void
  onFolderContextMenu: (e: React.MouseEvent, folder: CanvasFolder) => void
  ...
  // Drag & drop
  draggedCanvasIds: Set<string> | null
  draggingFolderId: string | null
  dragOverFolderId: string | null
  onCanvasDragStart: (canvas: Canvas, e: React.DragEvent) => void
  onFolderDragStart: (folder: CanvasFolder, e: React.DragEvent) => void
  onDragOverFolder: (e: React.DragEvent, folderId: string) => void
  onDragLeaveFolder: () => void
  onDropOnFolder: (e: React.DragEvent, folderId: string) => void
  onCanvasDropReorder: (e: React.DragEvent, targetCanvas: Canvas) => void
  onDragEnd: () => void
}

export default function CanvasesTree({
  parentId,
  depth,
  folders,
  canvases,
    ...
    <>
      ...
      <AnimatePresence>
        {childCanvases.map((canvas) => {
          const isActive = h.activeCanvasId === canvas.id
          const isSelected = h.selectedCanvasIds.has(canvas.id)
          const isBeingDragged = h.draggedCanvasIds?.has(canvas.id) ?? false
          return (
            <motion.div
              key={canvas.id}
              layout
            ...
            >
              <div
                draggable
                onDragStart={(e) => h.onCanvasDragStart(canvas, e)}
                onDragEnd={h.onDragEnd}
                onDragOver={(e) => { e.preventDefault(); e.stopPropagation() }}
                onDrop={(e) => { e.preventDefault(); e.stopPropagation(); h.onCanvasDropReorder(e, canvas) }}
                onClick={(e) => h.onCanvasClick(canvas, e)}
                onContextMenu={(e) => { e.preventDefault(); h.onCanvasContextMenu(e, canvas) }}
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  <Workflow size={16} className="shrink-0 text-ink-muted" />
                  <span className="font-heading text-sm text-ink-secondary truncate">{canvas.title}</span>
                </div>
                <motion.button
                  whileHover={{ scale: 1.15 }}
                  whileTap={{ scale: 0.9 }}
                  onClick={(e) => { e.stopPropagation(); h.onCanvasDelete(canvas.id) }}
...
// Icon-only rendering for the collapsed sidebar — same folder/canvas tree, no drag & drop or
// context menus, just click-to-open (canvases) and click-to-expand (folders).
export function CollapsedCanvasesTree({
  parentId,
  depth,
  folders,
  canvases,
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {childCanvases.map((canvas) => (
          <motion.div key={canvas.id} layout initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }} style={{ marginLeft: indent }}>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
