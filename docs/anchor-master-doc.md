...
- Full WCAG AA contrast audit across all 13 themes: checked body/secondary/muted/faint text and button text against their surfaces, plus all 5 badge `-strong`-on-`-subtle` pairs, in every theme. Found and fixed 24 failing pairs — `ink-faint` (used as real small text in 125 places, not just decorative) was failing in 8 of 13 themes (`light`/`pink`/`sepia`/`forest`/`solarized`/`nord`/`dracula`/`ocean`), and one or more badge pairs failed in `pink`/`nord`/`dracula`/`solarized`/`sepia`/`forest`/`ocean` (`nord` and `ocean` were worst, with 4 and 2 failing pairs respectively) — all fixed by adjusting lightness only, preserving each token's hue, with a comment on every changed line recording the old value and its failing ratio (`src/theme.css`)
- Theming correctness pass: native `<input type="date">` calendar-picker glyphs (which sit outside the app's CSS color tokens — the OS/Chromium draws them via `color-scheme`) now correctly render light-on-dark across all 8 dark-surfaced themes (`dark`/`nord`/`dracula`/`solarized`/`ocean`/`deuteranopia`/`protanopia`/`tritanopia`), not just literal `dark`, via a shared `getNativeColorScheme()` helper in `src/utils/theme.ts`; the sidebar's "Colourblind" flyout submenu no longer clips off the bottom of the window (was anchored `top-0` off a trigger row that sits near the window's bottom edge — now `bottom-0`); Windows' native titlebar-overlay caption buttons no longer visually erase the title bar's bottom border (overlay height now 1px shorter than the bar so it stops short of that border row)
- Performance pass: `saveDatabase()` throttled to at most one full `db.export()`+write per 400ms (was a synchronous full-DB write on every single mutation, blocking the Electron main process — see Notes/Learnings), with a `flushDatabaseSave()` bypass for vault-switch and app-quit; `ProjectBoard`'s card/column mutations (create/move/update/delete) update local state directly from the IPC response instead of refetching and re-rendering the whole board; `cardsInColumn` is a memoized `Map` instead of re-filtering+sorting `cards` on every render; canvas shape nodes and edges are `React.memo`'d with their label-change callback shared via context instead of injected into `node.data` (which recreated every node's identity on every drag frame); `ProjectBoard`/`NotesPage`/`CanvasesPage` are `React.lazy`-loaded — cut the main renderer bundle from ~1.2MB to ~380KB gzip since Tiptap and the `@xyflow/react` canvas stack are now separate on-demand chunks
...
53. App connections / integrations — pull from or push to external services instead of everything being manually entered, e.g. a new to-do task can pull from an email, or GitHub repo issues can auto-add to a project board as cards. Needs its own design pass (auth/OAuth per service, sync direction, conflict handling) before implementation starts.
54. Complex/nested projects — today's Projects (see Data Model → Projects) are flat; add support for sub-projects within a project, for users whose work doesn't fit a single flat board. Current thinking is nesting projects inside projects, but the actual data model and UI aren't designed yet.
...
- **Native form controls (e.g. `<input type="date">`'s calendar-picker glyph) sit outside the CSS color-token system entirely.** They're drawn by the OS/Chromium according to the `color-scheme` CSS property, not `currentColor`/theme variables, so adding a new theme doesn't automatically make them legible — each dark-surfaced theme needs `color-scheme: dark` set explicitly (a check like `theme === 'dark'` silently misses every *other* dark theme, e.g. `ocean`/`nord`). Same blind spot applies to scrollbar/`prefers-color-scheme`-driven native chrome generally; audit these separately from the semantic-token pass.
- **A flyout submenu anchored `top-0`/off-parent-edge can clip off the viewport if its trigger row sits near that edge.** The sidebar's theme-menu-within-a-menu ("Colourblind" flyout) is the last row of a menu that itself opens *upward* from a footer icon near the window's bottom — so the trigger row ends up close to the bottom edge, and a submenu opening `top-0` (downward) ran out of room. Match the submenu's anchor to whichever direction the parent already has room in (here, `bottom-0` to grow upward alongside it) rather than defaulting to `top-0`.
- **Windows' `titleBarOverlay` paints an opaque rectangle for the full height you give it, edge-to-edge, behind the caption buttons — including over any border your own title bar draws at that same height.** Giving the overlay the identical height as the title bar's own `border-b` erases that border wherever the buttons sit. Fix: make the overlay 1px shorter than the bar so its bottom border row falls outside the overlay's paint region.
- **`sql.js` has no partial/incremental save — every `saveDatabase()` call serializes the *entire* database and writes it synchronously.** Calling that on every single mutation (as the app originally did) blocks the Electron main process — which also handles all other IPC/window events — for a duration that scales with total vault size, not the size of the edit. Throttling it (mark-dirty + write-at-most-once-per-interval, with an explicit unthrottled flush for vault-switch/quit) is the fix; don't reach for a real embedded DB with incremental writes unless throttling stops being enough.
- **A `@xyflow/react` custom node/edge needs a stable `data` object reference to benefit from `React.memo`, and injecting a callback into `data` via `nodes.map(...)` on every render defeats that** — the map creates a new object for every node on every render (including every frame of an unrelated node's drag), so even a memoized node component re-renders anyway. Share the callback via React context instead (consumed with `useContext` inside the node component) so `nodes`/`edges` can be passed straight through unmodified, and only the node that actually changed gets a new object identity.
- **A token named for "faint"/decorative use can end up carrying real text weight app-wide, and its contrast bar should be judged from actual usage, not the name.** `ink-faint` reads as a low-emphasis token, but grew into 125 call sites across the app, overwhelmingly `text-xs` labels/metadata — normal-size text needing the full 4.5:1 AA bar, not the 3:1 large-text exception a "faint" name suggests. It was failing 4.5:1 in 8 of 13 themes before the contrast audit caught it. When adding a new theme, check every token's *actual* Tailwind usage (`grep -rn "text-ink-faint"` etc.) before assuming a lenient bar applies — a name is not a contract.
