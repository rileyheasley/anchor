...
- **Home** — an overview dashboard: active project count, total points, completion %, a "Recent Projects" shortlist, and a recycle-bin nudge when items are pending permanent deletion.
...
- **This dev sandbox can't launch the Electron GUI** (`vite-plugin-electron` spawns `main.js` under plain Node here, not the real Electron binary — an environment limitation, not a code issue). To verify DB/query changes without a live app, run a standalone Node script against `sql.js` directly, pointed at a **copy** of the real `test-data/anchor.db` — exercises the actual migration path and query shapes against real data without risking the working dev database.
